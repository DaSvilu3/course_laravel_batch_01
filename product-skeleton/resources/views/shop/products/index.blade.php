<x-app-layout>
    <x-slot name="header">
        <h1 class="text-2xl font-bold tracking-tight text-ink-900 dark:text-white">{{ __('shop.products') }}</h1>
    </x-slot>
    ...
    <form method="GET" class="card mb-6 flex flex-wrap items-end gap-3 p-4">
        ...
        <a href="{{ route('products.index') }}" class="btn-secondary">{{ __('common.reset') }}</a>
    </form>

    @if ($products->isEmpty())
        <div class="rounded-2xl border border-dashed border-ink-300 py-16 text-center dark:border-ink-700">
            <p class="text-ink-500 dark:text-ink-400">{{ __('shop.no_products') }}</p>
        </div>
    @else
