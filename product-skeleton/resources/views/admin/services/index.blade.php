            ...
            <tbody class="divide-y divide-ink-100 dark:divide-ink-800">
                @forelse ($services as $service)
                    <tr class="hover:bg-ink-50 dark:hover:bg-ink-800/50">
                        ...
                        <td class="table-td">
                            <span class="badge {{ $service->is_active ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-200' : 'bg-ink-100 text-ink-600 dark:bg-ink-800 dark:text-ink-300' }}">
                                {{ $service->is_active ? __('common.active') : __('common.inactive') }}
                            </span>
                        </td>
                        <td class="table-td">
                            <div class="flex items-center gap-3">
                                <a href="{{ route('admin.services.edit', $service) }}" class="font-medium text-brand-600 hover:text-brand-500 dark:text-brand-400">
                                    {{ __('common.edit') }}
                                </a>
